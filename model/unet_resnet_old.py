...
class UNetDown(nn.Module):
    def __init__(self, in_channels, out_channels, dropout_val=None, n_heads=8, dim_head=None):
...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels, n_feat=256, dropout_val=None, n_heads=4, dim_head=None):
        ...
        self.down1 = UNetDown(n_feat, n_feat, None, n_heads, n_feat)
        self.down2 = UNetDown(n_feat, 2 * n_feat, dropout_val, n_heads, 2 * n_feat)
        self.down3 = UNetDown(2 * n_feat, 2 * n_feat, None, n_heads, 2 * n_feat)
        
        self.latent = nn.Sequential(nn.AvgPool2d(4), nn.SiLU())
        
        self.up0 = nn.Sequential(
            nn.ConvTranspose2d(2 * n_feat, 2 * n_feat, 1, 1),
            nn.GroupNorm(8, 2 * n_feat),
            nn.SiLU(),
        )
        self.up1 = UNetUp(4 * n_feat, 2 * n_feat, dropout_val, n_heads, 2 * n_feat) # 4 * n_feat for the skip connection
        self.up2 = UNetUp(4 * n_feat, n_feat, None, n_heads, n_feat)
        self.up3 = UNetUp(2 * n_feat, n_feat, dropout_val, n_heads, n_feat)
